import { Trip, Vertex } from "./types";

interface DailyBikeData {
    startDate: Date;
    endDate: Date;
    vertices: Array<Vertex>;
}
const NO_SEG = 30;
const dailyBikeFullData: DailyBikeData[] = [];
let bikeData: Array<Trip> = [];
const counts: number[] = [];
onmessage = function (e) {
    try {
        const { type, data } = e.data;
        if (type === "register-data") {
            bikeData = data.sort((a: Trip, b: Trip) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime())
            bikeData.forEach((feature: Trip) => {
            ...
            if (type === "get-daily-data") {
                ...
                if (!dailyBikeFullData[day]) {
                    const dailyBikeData = bikeData.filter((trip: Trip) => new Date(trip.startTime).getDate() === day);
                    const startDate = new Date(`${day} January 2024 00:00:00`);
                    const endDate = new Date(`${day} January 2024 23:59:59`);
                    const vertices: Vertex[][] = [];
                    dailyBikeData.forEach((trip: Trip) => {
...
function calculatePointsOnParaboloid({ start, end }: { start: Vertex, end: Vertex }) {
